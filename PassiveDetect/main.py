...
# Main script to run to run PassiveDetect
...
## Get params
params_table = mainloop.get_params_table_passive_detect()
params_table = mainloop.assign_rig_specific_params_passive_detect(rigname, 
    params_table)
params_table['current-value'] = params_table['init_val'].copy()

## Get trial types
trial_types = mainloop.get_trial_types('passive_detect_simple_noiti')

## Initialize the scheduler
scheduler = Scheduler.RandomStimPassiveDetect(trial_types=trial_types)
...
try:
    ## Initialize GUI
    if RUN_GUI:
        plotter = ArduFSM.plot.PlotterPassiveDetect(trial_types)
